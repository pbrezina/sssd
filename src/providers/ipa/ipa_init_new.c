...
static errno_t ipa_init_auth_ctx(struct be_ctx *be_ctx,
                                 struct ipa_id_ctx *id_ctx,
                                 struct ipa_options *ipa_options)
{
    struct sdap_auth_ctx *sdap_auth_ctx;
    ...
    ret = ipa_init_ipa_auth_ctx(ipa_options, ipa_options,
                                id_ctx, &ipa_auth_ctx);
    if (ret != EOK) {
                      ...
                      void **_module_data)
{
    ...
    ret = ipa_init_options(mem_ctx, be_ctx, &ipa_options);
    if (ret != EOK) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to init IPA options\n");
        return ret;
    }

    ret = ipa_init_id_ctx(ipa_options, be_ctx, ipa_options, &ipa_id_ctx);
    if (ret != EOK) {
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to init dyndns\n");
        goto done;
    }
    ...
    if (dp_module_auth_enabled(provider, module_name)) {
        ret = ipa_init_auth_ctx(be_ctx, ipa_id_ctx, ipa_options);
        if (ret != EOK) {
                           ...
                           struct dp_method *dp_methods)
{
    ...
    id_ctx = talloc_get_type(module_data, struct ipa_id_ctx);
    auth_ctx = id_ctx->ipa_options->auth_ctx;
                             ...
                             struct dp_method *dp_methods)
{
    ...
    hostid_ctx->ipa_opts = id_ctx->ipa_options;
                           ...
                           struct dp_method *dp_methods)
{
    ...
    return ipa_sudo_init(mem_ctx, be_ctx, id_ctx, dp_methods);
    */
    return ENOTSUP;
#else
