...
static errno_t ad_init_auth_ctx(struct be_ctx *be_ctx,
                                struct ad_options *ad_options)
{
    struct krb5_ctx *krb5_auth_ctx;
    errno_t ret;

    if (ad_options->auth_ctx != NULL) {
        /* Already initialized. */
        return EOK;
    }

    krb5_auth_ctx = talloc_zero(ad_options, struct krb5_ctx);
                     ...
                     void **_module_data)
{
    ...
    ret = ad_init_options(mem_ctx, be_ctx, &ad_options);
    if (ret != EOK) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to init AD options\n");
        return ret;
    }
    ...
    ret = ad_get_id_options(ad_options, be_ctx->cdb,
                            be_ctx->conf_path,
    ...
    ret = setup_tls_config(ad_id_ctx->sdap_id_ctx->opts->basic);
    if (ret != EOK) {
    ...
    ret = sdap_idmap_init(ad_id_ctx->sdap_id_ctx, ad_id_ctx->sdap_id_ctx,
    ...
    ret = sdap_id_setup_tasks(be_ctx, ad_id_ctx->sdap_id_ctx,
    ...
    ad_id_ctx->sdap_id_ctx->opts->sdom->pvt = ad_id_ctx;
    ...
    ret = sdap_refresh_init(be_ctx->refresh_ctx, ad_id_ctx->sdap_id_ctx);
                          ...
                          struct dp_method *dp_methods)
{
    ...
    ret = ad_init_auth_ctx(be_ctx, id_ctx->ad_options);
    if (ret != EOK) {
    ...
    auth_ctx = id_ctx->ad_options->auth_ctx;

    dp_set_method(dp_methods, DPM_AUTH_HANDLER, NULL, NULL, auth_ctx,
                  struct krb5_auth_ctx, void, struct dp_reply_std);
                          ...
                          struct dp_method *dp_methods)
{
    ...
    return ad_sudo_init(mem_ctx, be_ctx, id_ctx, dp_methods);
    */
    return ENOTSUP;
#else
