...
static const char *get_req_name(struct test_ctx* test_ctx)
{
     const char *req_name;
...
static void test_add_del_req(void **state)
{
    ...
    req_name = get_req_name(test_ctx);
    ...
    ret = dp_req_table_add(test_ctx->table, req_name, req, sbus_req);
    assert_int_equal(ret, EOK);

    is_present = dp_req_table_has_key(test_ctx->table, req_name);
    assert_true(is_present);

    tv = dp_req_table_lookup(test_ctx->table, req_name);
    assert_non_null(tv);

    dp_req_table_del(test_ctx->table, req_name);

    tv2 = dp_req_table_lookup(test_ctx->table, req_name);
    assert_null(tv2);
...
static void test_mult_req(void **state)
{
    ...
    ret = dp_req_table_add(table, req_name, req, sbus_req);
    assert_int_equal(ret, EOK);
    ...
    tv2 = dp_req_table_lookup(table, req_name);
    assert_null(tv2);
...
static void test_destructor_req(void **state)
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        ret = dp_req_table_add(table, req_name, req, sbus_req[i]);
        assert_int_equal(ret, EOK);
    }
