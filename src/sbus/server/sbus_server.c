...
static const char *
sbus_server_get_socket_address(TALLOC_CTX *mem_ctx, bool use_symlink)
{
    unsigned long pid;
    ...
    pid = getpid();
    return talloc_asprintf(mem_ctx, "%s.%lu", SSS_MASTER_ADDRESS, pid);
}
                   ...
                   sbus_server_on_connection_data on_conn_data)
{
    ...
    dbus_server = sbus_server_setup_dbus(sbus_server, ev, use_symlink,
                                                   uid, gid, &symlink);
    if (dbus_server == NULL) {
